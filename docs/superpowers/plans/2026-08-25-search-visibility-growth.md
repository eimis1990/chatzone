...
- [x] Export page-filtered queries for the top 10 opportunity URLs listed in the
      page queue below, using the latest complete 28 days and previous 28 days.
- [x] Retain page/query clicks, impressions, CTR, and position; if using the
      Search Console API, group by both `page` and `query`.
- [x] Add a branded query classification (`loqara`, spelling variants) without
      assuming hidden/anonymized queries are non-brand.
- [x] Record one owner URL for every material query cluster.
...
**Completed 2026-08-26:** live GSC exact-page filters compared 2026-07-28 →
2026-08-24 with 2026-06-30 → 2026-07-27 for all eleven URLs represented by the
top ten queue entries. `docs/seo-data/2026-08-26-gsc-page-query-ownership.md`
records page totals, visible queries, privacy-threshold limits, branded/unknown
classification, and one owner per material cluster. The recommendation and
shopping-assistant URLs currently have distinct visible query clusters, so the
evidence does not support consolidation yet.

### Task 0.3 — Obtain actionable Bing AI data

**Suggested DRI:** SEO operator

- [x] Export page-level citation activity.
- [x] Export grounding queries and record the cited URL for each material query.
- [x] Separate Bing web-search clicks from AI citations in all reporting.
- [x] Establish a cited-page-diversity baseline: unique cited URLs and the share
...
**Completed 2026-08-26:** the live three-month Bing views (2026-05-26 →
2026-08-25) show 296 AI citation events, 13 sampled cited URLs, and 12 sampled
grounding queries. Every sampled query was drilled into its cited owner URL.
`docs/seo-data/2026-08-26-bing-ai-page-query-baseline.md` records the mapping,
the sample caveat, a 33.1% top-URL share, and Bing Web Search separately at 1
click / 74 impressions.
...
- [x] Create an organic landing-page report showing visits, engaged visits, CTA
      opens, signup starts, and completions.
- [x] Verify events in production with one controlled organic/referrer test and
...
and application commit `94a9e4b` is live. Six GA4 custom dimensions and the
`Organic landing-page signup funnel` Explore are configured. A controlled
`seo_test / organic` article journey appeared in GA4 Realtime through
`signup_succeeded`, persisted the matching first touch, and was verified in the
owner card and CSV. Task 0.4 now passes. After explicit owner confirmation, the
internal test signup was deleted and a production query returned zero matches.
...
- [x] Verify time-sensitive vendor features and prices against current official
...
**Completed 2026-08-26:** the capability matrix and Loqara public-claim
corrections are recorded in
`docs/seo-data/2026-08-20/capability-claim-audit.md`. The dated third-party
review in `docs/seo-data/2026-08-26-vendor-source-review.md` checks current
official pricing/docs for Zendesk, Intercom, Tidio, Gorgias, Re:amaze, Crisp,
Chatbase, Voiceflow, HubSpot, and ElevenLabs. It removed unsupported Zendesk
estimates and corrected obsolete Gorgias double-charge/rate and Crisp AI-gating
claims across the priority comparison pages.
...
- [x] Production `next build` — compiled and generated 237 static pages.
- [x] `git diff --check` — passed.
- [x] Database migration verification — applied only
      `20260826085019_signup_acquisition_attribution.sql` to the linked project;
      verified all seven nullable columns and matching local/remote migration
      history. The older unrelated pending migration was not applied.
- [x] Production deployment verification — commit `94a9e4b` is on `main`; the
      live homepage returned HTTP 200 and its deployed JavaScript contains the
      new first-touch and funnel event identifiers.
- [x] GA4 production configuration — registered `landingPath`,
      `acquisitionSource`, `acquisitionMedium`, `referrerHost`, `campaign`, and
      `ctaSource`; created Explore `Organic landing-page signup funnel` with the
      organic segment, five steps, and landing-path breakdown.
- [x] Controlled production journey — GA4 Realtime received CTA/open/start/
      submit/succeeded events from the tagged e-commerce article; the accepted
      signup stored `nav`, `/blog/best-ai-chatbot-for-ecommerce`,
      `seo_test / organic`, and `phase0`, all visible in the owner card and CSV.
      The authenticated `/owner/signups` visit remained outside public analytics.
- [x] Production test cleanup — after explicit owner confirmation, deleted only
      `hello+seo-test-20260826@loqara.com`; a follow-up database query confirmed
      zero matching signup rows remain.

### Phase 0 review gate

- [x] Tasks 0.1–0.6 pass their acceptance criteria.
- [x] Baseline and conversion reports are accessible to the owner.
- [x] Each Phase 1/2 page has a query owner and a named business outcome.
- [x] Review recorded in the program log below.

**Gate passed 2026-08-26:** reproducible Google/Bing baselines, page/query
ownership, Bing cited-page/query evidence, production organic-funnel attribution,
capability truth, current vendor sources, and the refresh-first operating loop
are in place. The baseline/source files are stored under `docs/seo-data/`; the
conversion dimensions are also visible in the production owner signup view and
CSV. Phase 1 may start without changing core bot, commerce, billing, or chat
behavior.

### Phase 1/2 query-owner and business-outcome map

`Measured` means the 2026-08-26 exact-page GSC capture exposed a visible query or
page-level intent signal. `Provisional` means the existing URL is assigned as the
single owner now, but Task 1.3 must confirm its page-filtered queries before any
snippet/content edit. Privacy-hidden queries remain unknown.

| Owner URL | Query/reader job owned | Evidence | Named business outcome |
|---|---|---|---|
| `ai-customer-service-statistics` | AI customer-service statistics and benchmark selection | Measured page; visible queries privacy-suppressed | Earn reuse/link value and move qualified readers into an evidence-led product trial |
| `new-ai-chatbots-2026` | New/current AI-chatbot developments in 2026 | Measured page; visible queries privacy-suppressed | Turn freshness discovery into platform/voice evaluation and a Get Started open |
| `best-ai-chatbot-for-ecommerce` | Best AI chatbot for an e-commerce store | Provisional; verify in Task 1.3 | Convert active vendor shortlist readers into signup starts |
| `how-to-choose-ai-support-agent` | How to evaluate/choose an AI support agent | Provisional; verify in Task 1.3 | Move checklist users from evaluation to Get Started |
| `multilingual-ai-customer-support` | Multilingual AI support requirements and fit | Provisional; verify in Task 1.3 | Qualify English/Lithuanian-fit stores and avoid unsuitable signups |
| `live-chat-vs-ai-chatbot-ecommerce` | Live chat versus AI chatbot for e-commerce | Provisional; verify in Task 1.3 | Help category-comparison readers choose the Loqara-shaped option and open signup |
| `where-is-my-order-ai` | AI automation for “where is my order?” | Provisional; verify in Task 1.3 | Generate qualified WooCommerce/Magento order-lookup trials |
| `best-chatbot-platforms` | Broad chatbot-platform selection | Measured page | Generate vendor-comparison Get Started opens without overstating fit |
| `zendesk-ai-review` | Zendesk AI features, pricing model, and review intent | Measured page; five Bing AI query variants confirmed | Convert right-sized alternative seekers and earn cited buying-guide authority |
| `shopify-ai-assistant-guide` | Shopify-admin AI, Magic, and Sidekick | Measured page | Route merchants who need customer-facing help to the separate storefront solution |
| `shopify-inbox-vs-ai-chatbot` | Shopify Inbox versus storefront AI support | Measured page | Generate qualified Shopify product-search/support signup starts |
| `ai-chatbot-for-beauty-skincare-stores` | Beauty/skincare-store AI assistant use cases | Measured growth page | Generate separately tracked vertical CTA opens and qualified trials |
| `ai-chatbot-for-furniture-stores` | Furniture-store chatbot template/use cases | Measured template query | Earn template reuse and separately tracked furniture-store CTA opens |
| `add-ai-agent-to-woocommerce` | Add/setup an AI agent on WooCommerce | Measured winner | Protect setup-intent clicks and convert them into WooCommerce trials |
| `ai-product-recommendation-chatbot` | Conversational product-recommendation implementation | Measured Google and Bing owner | Generate catalog/product-search trials and preserve the Bing product-advice citation owner |
| `conversational-ai-shopping-assistant` | Shopping-assistant category/explainer intent | Measured separate query cluster | Educate broader category traffic and route it to recommendation/product trial content |
...
- [x] Export and record the page's current queries before editing.
- [x] Replace stale or secondary statistics with current primary sources.
- [x] Add relevant 2026 research only where the original source supports the
      exact claim; record source dates and sample scope.
- [x] Add a transparent selection/methodology note and explain limitations.
- [x] Add one downloadable or reusable chart/table with descriptive alt text.
- [x] Add original Loqara evidence only if the sample is real, anonymized, and
      disclosed; otherwise do not imply a benchmark exists.
- [x] Rewrite title/description to match the measured query and page value.
- [x] Add at least four relevant contextual inbound links from existing posts.
...
**Local progress 2026-08-26:** refreshed all 20 figures from current original
Gartner, Salesforce, and Zendesk research; exposed populations, measurement
types, vendor-publisher limits, and a reusable 20-row CSV; added four contextual
inbound links. The title was retained because the current exact-page GSC view
exposes no query rows; the description now promises the evidence/scope value
instead of unsupported universal savings. Full evidence and exact old/new copy
are in `docs/seo-data/2026-08-26-ai-customer-service-statistics-refresh.md`.
Deployment, recrawl, and timed reviews remain open.

### Task 1.2 — Refresh `new-ai-chatbots-2026`

**Baseline:** 37 impressions, position 6.16, zero page-table clicks.

- [x] Confirm the queries and whether the current title matches their intent.
- [x] Add a visible “updated August/September 2026” review section only after a
      material vendor/source review.
- [x] Add a concise change log: what changed, for whom it matters, and the review
      method.
- [x] Verify all vendor features, prices, and availability against official
      sources and date the comparison.
- [x] Test one title/description change; log the exact previous and new values.
- [x] Add contextual internal links from platform and comparison hubs.
...
**Local progress 2026-08-26:** retained the title because exact-page GSC query
rows are suppressed and the small current sample still averages position 5;
tested a more specific description instead. The page now has a visible change
log and an official-source review of Nvidia ACE/ChatRTX, OpenAI GPT-Live and
commerce discovery, and ElevenAgents availability/pricing. It also replaces an
unsupported vertical-performance claim with a testable workflow-fit framework
and has two new hub backlinks. Evidence and exact copy are in
`docs/seo-data/2026-08-26-new-ai-chatbots-refresh.md`. Deployment, recrawl, and
timed reviews remain open.
...
- [x] Export page-filtered queries for every page.
- [x] Classify each as `keep`, `snippet refresh`, `content refresh`, or
      `consolidation candidate`.
- [x] Make no title change when the query sample is too small to diagnose CTR.
- [x] For approved refreshes, strengthen the direct answer, decision criteria,
      limitations, and commercial next step.
- [x] Add missing two-way contextual links within the relevant cluster.
- [x] Log one change set per URL and schedule 7/28-day reviews.
...
**Local progress 2026-08-26:** classified all six URLs from exact-page live GSC
comparisons. Four remain `keep`; `best-chatbot-platforms` retains its Phase 0
official-source refresh; and `multilingual-ai-customer-support` received an
accuracy-led content refresh because it promised universal language coverage
despite Loqara supporting English and Lithuanian. No title changed on hidden or
tiny query samples, no consolidation was justified, and multilingual/live-chat
now have a two-way contextual link. The per-URL log and relative 7/28-day
schedule are in `docs/seo-data/2026-08-26-phase1-watch-list-triage.md`.

### Phase 1 review gate

- [x] `npm run audit:blog` passes without new objective failures.
- [x] External claims, internal links, image alt text, capability statements,
...
**Local gate progress 2026-08-26:** content audit passes with zero errors and
only the pre-existing shopping-assistant inbound-link warning. The edited pages'
claims, links, alt text, author metadata, dates, and Loqara capability boundaries
have been reviewed. Rendering, production smoke checks, recrawl requests, and
calendar dates remain deployment-dependent.
...
| Phase 0 gate | 2026-09-08 | [x] | Passed early on 2026-08-26: Tasks 0.1–0.6 accepted; owner/outcome map recorded; Phase 1 unblocked |
