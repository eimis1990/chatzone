-- Complete 20260802130000: the color-over-title ordering must also apply to
-- the field_match CTE's internal top-80 cut, or a color-matched product with
-- no title token (baldaila.lt's SKU-titled Arcadova) is dropped there — after
-- ~190 title-matched sofas — before the final ORDER BY ever sees it.
...
as $$
  ...
  field_match as (
    select fr.id, fr.title_matches, fr.color_matches, fr.attribute_matches,
           ...
           row_number() over (
             order by fr.color_matches desc, fr.title_matches desc, fr.attribute_matches desc,
