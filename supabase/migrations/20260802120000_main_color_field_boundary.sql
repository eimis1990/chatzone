-- Anchor the main-color extraction to a FIELD BOUNDARY (neutral matcher only).
--
-- The bare pattern 'Spalva: (…)' matches inside compound attribute names:
-- baldaila.lt's "Porankių Spalva: Ąžuolo spalva, Juoda spalva" (ARMREST color)
-- was extracted as the product's main color, so a white sofa whose whiteness
-- is a derived "Color:" attribute ranked as oak/black. Require the label to
-- start a field: beginning of doc, a new line, "; " (attribute separator), or
-- the "Attributes: " line prefix. "Kojų spalva"/"Porankių Spalva" no longer
-- match. `match_products_verskis` keeps its pattern: Verskis docs always list
-- the main `Spalva` first, so the first-match extraction is already correct.
