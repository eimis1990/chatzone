-- Rank main-color matches above title-token matches within the covered tier
-- (neutral matcher only).
--
-- For a color-constrained query ("balta sofa"), full coverage already
-- guarantees the product noun appears SOMEWHERE in the doc (title, category,
-- or description), so the title-token count mostly re-states that — while the
-- color field is the precise constraint signal. Title-first ordering pushed
-- baldaila.lt's Arcadova (a white sofa titled with a bare SKU, "sofa" only in
-- its category) below every title-matched sofa, out of the 24-candidate pool.
-- Color-first keeps every verified-white product ahead of white-mentioned-in-
-- description ones; title count remains the tiebreak within a color band, so
-- colorless queries (color_matches = 0 everywhere) are unaffected.
