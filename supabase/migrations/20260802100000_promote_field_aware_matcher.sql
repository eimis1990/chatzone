-- Promote the field-aware attribute/color ranking (proven on Verskis since
-- 20260717115738) into the provider-neutral `match_products`. WooCommerce /
-- Shopify / Magento shoppers asking "balta sofa" were getting random-color
-- sofas: the neutral matcher ranked by vector + FTS + whole-title tokens only,
-- and embeddings barely encode color. The field-aware ranking acts purely on
-- the shared doc format (`Attributes: …`, main `Spalva:|Color:|Colour:` field)
-- that every indexed provider emits via buildDoc, so it is not store-specific.
-- After this, `match_products` and `match_products_verskis` are identical; the
-- verskis alias can be collapsed in a later cleanup migration.
