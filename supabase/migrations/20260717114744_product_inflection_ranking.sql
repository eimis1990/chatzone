-- Lithuanian product searches commonly arrive inflected ("baltos kėdės",
-- "virtuvinių kėdžių"), while Verskis attributes are indexed in their
-- nominative forms ("Spalva: Balta", "Kėdės"). The `simple` FTS dictionary does
-- not stem Lithuanian, so exact attribute matches could be absent from the top
-- vector candidates even though they exist in the catalog.
--
-- Add a compact folded-prefix retrieval tier. Three-character prefixes are
-- deliberately used because Lithuanian inflection can mutate the fourth letter
-- (`kėdė` -> `kėdžių`). Prefix coverage ranks before the RRF/vector score,
-- while exact title matches remain the strongest signal.
...
as $$
  ...
  prefix_toks as (
    select distinct left(t, 3) as prefix
    from toks
    where length(t) >= 3
  ),
  -- Every complete query token appears in the title: strongest possible signal.
  tmatch as (
    select b.id
    from base b
  ...
  prefix_raw as (
    ...
    from base b
    join prefix_toks pt
      on position(pt.prefix in public.fold_lt(coalesce(b.doc, ''))) > 0
