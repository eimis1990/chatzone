...
export function ChatWindow({ publicKey, config }: ChatWindowProps) {
   ...
   * so the TTS button can use them. We replace messages that have a temporary
   * client-side id (generated by `generateId`) with the real server-issued UUID.
   */
  const syncMessageIds = useCallback(
    async (convId: string) => {
      ...
      try {
        ...
        setMessages((prev) => {
          // Build a content→id map for assistant messages from the server response.
          const serverAssistant = data.messages!.filter((m) => m.role === 'assistant')
          // Match by content (exact) — stable once streaming is done.
          return prev.map((m) => {
        ...
        // Non-critical — TTS button just won't appear for unresolved messages.
      }
    },
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        // Sync real message ids after streaming is complete (for TTS).
        if (convId) {
    ...
    [streaming, conversationId, leadDismissed, config.leadCapture.enabled, publicKey, syncMessageIds]
  )
