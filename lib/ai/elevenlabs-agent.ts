...
export function buildDisplayToolConfig(config: Bot['config']) {
  ...
  return {
    ...
    description:
      ...
      'ONLY candidate ids that ARE the requested product type AND satisfy EVERY active hard ' +
      'constraint; missing facts are unverified and must not be displayed. Order best matches first ' +
      'and pass no more than 20 ids. For a tight multi-constraint request, a small verified set is ' +
      'better than padding with weak matches. If NO candidate is genuinely the requested type, do ' +
      'not call this tool at all — tell the user honestly what the catalog does and does not have. ' +
      (displayGuidance || ''),
    expects_response: true,
    parameters: {
      type: 'object' as const,
