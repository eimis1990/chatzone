...
When people search for the "Nvidia AI chatbot," they usually mean ChatRTX — the free Windows app that let anyone with an RTX graphics card run a local chatbot over their own documents, with retrieval-augmented generation and no cloud in sight. It was a genuinely clever demo of local AI. It's also, [as of January 2026, deprecated](https://github.com/NVIDIA/ChatRTX): Nvidia has stopped maintaining it. That tells you something important about where Nvidia thinks the value is — and it isn't consumer chat apps.

The real Nvidia story is two layers deeper. **ACE** (Avatar Cloud Engine) is its suite for "digital humans" — animated, speaking, emoting AI characters aimed at customer service, gaming, and healthcare, [generally available as microservices](https://nvidianews.nvidia.com/news/digital-humans-ace-generative-ai-microservices) since 2024. **NIM** (Nvidia Inference Microservices) is the delivery mechanism: pre-packaged, optimised AI models that developers deploy in the cloud, on-premises, or on RTX hardware — including small language models built to run entirely on-device. Nvidia's bet is that the next wave of chatbots and avatars, whoever brands them, runs on its stack.
...
If one capability jumped from "impressive demo" to "quietly everywhere" in the past year, it's voice. Two companies did most of the pushing. **ElevenLabs** turned its ultra-realistic voices into a full conversational agent platform — speech recognition, natural turn-taking, tool calling, support for [70+ languages](https://elevenlabs.io/agents) — and raised a [$500 million Series D at an $11 billion valuation in February 2026](https://elevenlabs.io/blog/series-d), with millions of agents deployed on its platform. **OpenAI** made its [Realtime API generally available](https://openai.com/index/introducing-gpt-realtime/) with models built for production voice agents: sub-second responses, phone-system integration via SIP, and per-minute costs that keep falling.
...
The word "agentic" is overused, but the shift underneath it is real: the frontier moved from chatbots that *answer* to agents that *do*. The clearest signal came from commerce itself. OpenAI launched Instant Checkout in ChatGPT — buy without leaving the conversation — signed up major platforms, and then [walked parts of it back within months](https://www.cnbc.com/2026/03/24/openai-revamps-shopping-experience-in-chatgpt-after-instant-checkout.html) when the data came in: Walmart, for one, [reportedly saw in-chat checkout converting roughly three times worse](https://www.cnbc.com/2026/03/20/open-ai-agentic-shopping-etsy-shopify-walmart-amazon.html) than simply sending the customer to walmart.com, even as ChatGPT referrals brought in about twice the new-customer rate of search, according to CNBC. The lesson wasn't "agents failed"; it was that *discovery* belongs in the conversation and *trust-heavy actions* belong where the customer expects them.
